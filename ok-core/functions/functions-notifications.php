...
class OkNotificationManager
{
    ...
    private static function ensureSchema(): void
    {
        ...
        $legacyRows = $ok_db->get_results(
            'SELECT id, for_user_id, read_by_users, created_at FROM ' . self::TABLE_NOTIF . ' WHERE for_user_id IS NOT NULL AND for_user_id != "" LIMIT 500'
        );

        if ($legacyRows) {
            foreach ($legacyRows as $row) {
                $targets = array_filter(array_map('intval', explode(',', (string)$row->for_user_id)));
                $readBy = array_filter(array_map('intval', explode(',', (string)($row->read_by_users ?? ''))));

                foreach ($targets as $uid) {
                    $isRead = in_array((int)$uid, $readBy, true) ? 1 : 0;
                    $readAt = $isRead ? ($row->created_at ?? date('Y-m-d H:i:s')) : null;
                    $ok_db->query(
                        'INSERT IGNORE INTO ' . self::TABLE_RECIPIENTS . ' (notification_id, user_id, is_read, read_at, created_at) VALUES (?, ?, ?, ?, ?)',
                        [(int)$row->id, (int)$uid, $isRead, $readAt, $row->created_at ?? date('Y-m-d H:i:s')]
                    );
                }
            }
        }
    ...
    public static function enforcePollRateLimit(): void
    {
        ...
        $limit = (int)self::getOption('notif_poll_rate_limit', '60');
        if ($limit < 10) $limit = 10;
        if ($limit > 300) $limit = 300;

        if (count($_SESSION['_ok_notif_poll_times']) >= $limit) {
...
function ok_handle_notification($payload): void
{
    if (is_array($payload)) {
        ...
        $author = $payload['author_id'] ?? null;
        if ($message !== '') {
            ok_add_notification($message, $type, $targets, $link, $author);
        }
        return;
    }

    $msg = trim((string)$payload);
    if ($msg !== '') {
        ok_add_notification($msg, 'info');
    }
}
